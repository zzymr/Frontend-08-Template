...
### 重点在机上，不在状态上，之所以用有限状态机去处理一些问题，是因为有限状态机里面的每一个机器都是解藕的，每个状态都是一个独立的机器的思想，让有限状态机变成了一个强有力的抽象机制，有限状态机最大的特点就是，每一个状态都是一个机器

+ 每一个状态都是一个机器
    + 在每一个机器里，我们可以做计算、存储、输出。意味着我们编写状态机的代码时候，可以完全忽略其他状态机的逻辑，只关心本状态要处理什么，游戏中广泛应用AI,用来简化逻辑。
    + 所有这些机器接受的输入是一致的； 要么都接受字符串，要么都接受整数
    + 状态机的每个机器本身是不能再有状态，如果用函数表示，这个状态机里面的这些函数都是纯函数，不能有副作用。比如针对其他的像输出，改变变量不属于副作用，这里的副作用指的的机器不能再收到外部的输入控制，输出是可以的。用函数表示状态机的时候，可以往外写，不能再从外边读变量进来，一旦读取影响状态机本身的状态切换逻辑，会出很大的问题
...
### TCP和IP的基础知识
+ 流   没有明显分割单位的一种东西，只保证前后的顺序是正确的，类似水流
+ 端口 TCP协议被计算机中的软件所使用，每个软件从网卡那数据，哪个数据是分配给哪个软件的，计算机通过端口把接到的数据包分给各个应用 对应node require('net')
+ require('net')
+ 包    可大可小，取决于整个网络中间设备的传输能力
...
----------------
1. 字符串采用unicode 或者ASCII编码传递
2. HTTP在TCP的上层，流淌在TCP流中所有的内容都可以视为是字符
3. headers 行数不固定，结束是以一个空行为标志结束的
4. body部分由Content-Type决定的，规定了什么格式 body就用什么格式，body也是一个kv 结构
5. 所有HTTP中的换行按照规定都是\r \n 两个字符组成的换行符，解析时候容易出错        
...
+ send的过程中会逐步收到信息，设计reponse parse 而不是Response类，parse通过逐步接收Response信息来构造Response对象的各个不同的部分
### response格式
+ HTTP/1.1(版本) 200(http的状态码) OK(http的状态文本)           status line
    + 500 系列服务器错误，404找不到网页，200 ok，301，302，304相关的需要进一步处理
...
-------------
+ body  里面允许任何字符  典型的node中chunked body 由（16进制的数字单独占一行，后面内容部分，16进制数字），直到16进制的0，它最后得到空的 chunk
    + 26 16进制的数字
